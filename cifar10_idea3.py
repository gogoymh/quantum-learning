...
for epoch in range(300):
    runnning_loss = 0
    new_model.train()
    for x, y1, y2 in new_trainloader:
        new_optimizer.zero_grad()
              
        #x = x[y1.long() == y2.long()]
        #y = y1[y1.long() == y2.long()]
        
        Bool = y1.long() == y2.long()
        
        re_B = (~Bool).sum()
        re_labels = torch.zeros(re_B, 10).to(device).scatter_(1, y1[~Bool].unsqueeze(1).to(device), 0.5)
        re_labels += torch.zeros(re_B, 10).to(device).scatter_(1, y2[~Bool].unsqueeze(1).long().to(device), 0.5)
        
        output = new_model(x.float().to(device))
        #print((torch.isnan(output)).sum())
        #loss = criterion(output, y.long().to(device))
        loss = F.cross_entropy(output[Bool], y2[Bool].long().to(device), reduction='sum')
        loss += soft_ce(output[~Bool], re_labels).sum()
        loss /= x.shape[0]
        #loss = loss[y1.long() == y2.long()].mean()
        loss.backward()
